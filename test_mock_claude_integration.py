...
async def test_lawmate_with_mock_claude():
    """
    LawMate API와 Mock Claude 서비스 통합 테스트
    - Claude API 호출 없이 테스트 가능
    """
    print("\n=== LawMate + Mock Claude 통합 테스트 ===\n")
    law_service = LawDataService()
    mock_claude_service = MockClaudeService()
    ...
    print("테스트 사례:")
    print(test_case)
    print("\n1. Mock Claude 서비스로 법률 문제 분석 중...\n")
    
    # 1. Mock Claude 서비스로 법률 문제 분석
    analysis = await mock_claude_service.analyze_legal_issue(test_case)
    ...
    # 6. 상세 법률 상담 답변 생성 (Mock)
    print("\n6. Mock Claude 서비스로 상세 법률 상담 답변 생성 중...\n")
    
    consultation_response = await mock_claude_service.generate_legal_consultation(
    ...
    document_draft = await mock_claude_service.generate_document_draft("내용증명", case_info, recipient_info)
...
if __name__ == "__main__":
    asyncio.run(test_lawmate_with_mock_claude())
